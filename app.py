...
def find_weekday_in_month(year, month, js_day_of_week, occ_str):
    """Find the Nth or last occurrence of a JS weekday (0=Sun) in a month"""
...
def home():
    ...
    for cal in calendars:
        query(cur, is_postgres,
              "SELECT COUNT(*) as total FROM tasks WHERE folder_id = %s AND user_id = %s",
              (cal["id"], current_user.id))
        total = cur.fetchone()["total"]
        query(cur, is_postgres,
              "SELECT COUNT(*) as done FROM tasks WHERE folder_id = %s AND user_id = %s AND done = 1",
              (cal["id"], current_user.id))
        done = cur.fetchone()["done"]
        query(cur, is_postgres,
              "SELECT COUNT(*) as overdue FROM tasks WHERE folder_id = %s AND user_id = %s AND done = 0 AND due_date IS NOT NULL AND due_date != '' AND due_date < %s",
              (cal["id"], current_user.id, today))
        overdue = cur.fetchone()["overdue"]
        query(cur, is_postgres,
              "SELECT COUNT(*) as warning FROM tasks WHERE folder_id = %s AND user_id = %s AND done = 0 AND due_date IS NOT NULL AND due_date != '' AND due_date >= %s AND due_date <= %s",
              (cal["id"], current_user.id, today, tomorrow))
...
def delete_calendar(calendar_id):
    conn, is_postgres = get_db()
    cur = conn.cursor()
    # Delete completions for tasks in this calendar
    query(cur, is_postgres,
          "DELETE FROM task_completions WHERE task_id IN (SELECT id FROM tasks WHERE folder_id = %s AND user_id = %s)",
          (calendar_id, current_user.id))
...
def view_calendar(calendar_id):
    ...
    # Build completed_dates map: task_id -> [date1, date2, ...]
...
def api_tasks():
    ...
    if is_postgres:
        ...
        cur = conn.cursor()
    query(cur, is_postgres,
          "SELECT * FROM tasks WHERE folder_id = %s AND user_id = %s AND due_date = %s",
          (calendar_id, current_user.id, date_str))
    tasks = cur.fetchall()
    cur.close()
    conn.close()
...
def api_edit_task(task_id):
    ...
    cur = conn.cursor()
    query(cur, is_postgres,
          "UPDATE tasks SET task = %s, due_date = %s, recurrence = %s WHERE id = %s AND user_id = %s",
...
def api_toggle_task(task_id):
    ...
    if is_recurring and toggle_date:
        ...
        query(cur, is_postgres,
              "SELECT id FROM task_completions WHERE task_id = %s AND completed_date = %s",
              (task_id, toggle_date))
        existing = cur.fetchone()
        if existing:
            # Un-complete this date
            query(cur, is_postgres,
                  "DELETE FROM task_completions WHERE task_id = %s AND completed_date = %s",
                  (task_id, toggle_date))
            result["done"] = 0
        else:
            # Complete this date
            try:
                query(cur, is_postgres,
                      "INSERT INTO task_completions (task_id, completed_date) VALUES (%s, %s)",
                      (task_id, toggle_date))
            except:
                conn.rollback()
            result["done"] = 1
        ...
        # Non-recurring: toggle done field as before
